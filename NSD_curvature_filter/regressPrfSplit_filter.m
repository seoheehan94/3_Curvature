    ...
    for roinum=1:length(rois)
        ...
        for isplit=1:nsplits
            ...
            for ivox=1:nvox(roinum)
                ...
                r2len{roinum}(isplit,ivox) = rsquared(voxCurvResidual{roinum}(isplit,ivox,1:sum(splitImgTrials(isplit,:))), roiBetas{roinum}(ivox,imgTrials>0));
                
                %resid r2 within split
                %residuals of full orientation model:
                lenResidCurvResidual = voxBetas' - squeeze(voxCurvResidCurvCoef{roinum}(isplit,ivox,:))'*voxPrfCurvSample';
                voxCurvResidCurvR2{roinum}(isplit,ivox) = rsquared(lenResidCurvResidual(1:sum(splitImgTrials(isplit,:))), roiBetas{roinum}(ivox,imgTrials>0));
                
                %prediction of full orientation model:
                lenPredCurvResidual = voxBetas' - squeeze(voxCurvPredCurvCoef{roinum}(isplit,ivox,:))'*voxPrfCurvSample';
                voxCurvPredCurvR2{roinum}(isplit,ivox) = rsquared(lenPredCurvResidual(1:sum(splitImgTrials(isplit,:))), roiBetas{roinum}(ivox,imgTrials>0));
        ...
        for isplit=1:nsplits
            ...
            for ivox=1:nvox(roinum)
                ...
                r2lenSplit{roinum}(isplit,ivox) = rsquared(voxCurvResidualSplit{roinum}(isplit,ivox,1:sum(splitImgTrials(isplit,:))), roiBetas{roinum}(ivox,imgTrials>0));
                
                %corr between splits
                pearsonRlen{roinum}(isplit,ivox) = corr(voxBetas,(squeeze(voxCurvCoef{roinum}(nsplits-isplit+1,ivox,:))'*voxPrfCurvSample')');
